...
for($i=1;$i<=8;$i++) {
	$Cartoon = $arrayOne['Video_ID']."|".$Cartoon;
}
//Cartoon 部分数据
...
for($i=1;$i<=8;$i++){
	$TimeSeries =$arrayTwo['Video_ID']."|".$TimeSeries;
}
//番剧部分数据
...
for ($i=1;$i<=8;$i++) { 
	$Dance = $arrayFour['Video_ID']."|".$Dance;
}
//舞蹈部分
...
for ($i=1;$i<=8;$i++) { 
	$Science = $arraySix['Video_ID']."|".$Science;
}
//科学技术部分
...
for ($i=1;$i<=8;$i++) { 
	$Entertainment = $arraySeven['Video_ID']."|".$Entertainment;
}
//娱乐部分
...
for ($i=1;$i<=8;$i++) { 
	$Hentai = $arrayEight['Video_ID']."|".$Hentai;
}
//鬼畜部分
...
for ($i=1;$i<=8;$i++) { 
	$Moive = $arrayNine['Video_ID']."|".$Moive;
}
//电影部分
...
for ($i=1;$i<=8;$i++) { 
	$TVplay = $arrayTen['Video_ID']."|".$TVplay;
}
//电视剧部分
$IDOutput = $Cartoon."-".$TimeSeries."-".$Music."-".$Dance."-".$Game."-".$Science."-".$Entertainment."-".$Hentai."-".$Moive."-".$TVplay;
echo $IDOutput;
?>
