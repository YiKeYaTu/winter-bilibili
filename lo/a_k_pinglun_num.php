...
for($i=1;$i<=8;$i++) {
	$Cartoon = $arrayOne['Video_Pinglun_Num']."|".$Cartoon;
}
//Cartoon 部分数据
...
for($i=1;$i<=8;$i++){
	$TimeSeries =$arrayTwo['Video_Pinglun_Num']."|".$TimeSeries;
}
//番剧部分数据
...
for ($i=1;$i<=8;$i++) { 
	$Music =$numThree['Video_Pinglun_Num']."|".$Music;
}
//音乐部分数据  
...
for ($i=1;$i<=8;$i++) { 
	$Dance = $arrayFour['Video_Pinglun_Num']."|".$Dance;
}
//舞蹈部分
...
for ($i=1;$i<=8;$i++) { 
	$Game = $arrayFive['Video_Pinglun_Num']."|".$Game;
}
//游戏部分
...
for ($i=1;$i<=8;$i++) { 
	$Science = $arraySix['Video_Pinglun_Num']."|".$Science;
}
//科学技术部分
...
for ($i=1;$i<=8;$i++) { 
	$Entertainment = $arraySeven['Video_Pinglun_Num']."|".$Entertainment;
}
//娱乐部分
...
for ($i=1;$i<=8;$i++) { 
	$Hentai = $arrayEight['Video_Pinglun_Num']."|".$Hentai;
}
//鬼畜部分
...
for ($i=1;$i<=8;$i++) { 
	$Moive = $arrayNine['Video_Pinglun_Num']."|".$Moive;
}
//电影部分
...
for ($i=1;$i<=8;$i++) { 
	$TVplay = $arrayTen['Video_Pinglun_Num']."|".$TVplay;
}
//电视剧部分
$PinglunnumOutput = $Cartoon."-".$TimeSeries."-".$Music."-".$Dance."-".$Game."-".$Science."-".$Entertainment."-".$Hentai."-".$Moive."-".$TVplay;
echo $pinglunnumOutput;
?>